...
int main() {
    ...
    if (pid < 0) { // Error occurred while creating the child process
        fprintf(stderr, "Fork failed");
        exit(1);
    }

    else if (pid == 0) {  // Child process executing the readSort function
        readSort(filename);
    }

    else {  // Parent process executing the readCheck function
        wait(NULL);
        readCheck(filename);
    }
